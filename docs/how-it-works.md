...
`package.json` declares a Pi manifest with `skills: ["./skills/pi"]`. Pi therefore loads only the platform-safe skills under `skills/pi/`; it does not load the Claude Code installer payload or the Claude-only reference workflow. The Pi skills use the native `subagent` tool and project-local `.pi/plans/` artifacts. The bundled extensions expose `loop_progress`, `loop_inventory`, `loop_evidence`, and `loop_limits` tools. `loop_limits` renders model context usage, session tokens/requests/cost, billing mode, and optional configured windows below the editor. Pi does not expose remote ChatGPT subscription quota data, so the panel never invents a percentage; it labels that status unavailable. Optional project configuration lives in `.pi/loop-limits.json`, for example `{"windows":[{"label":"Session budget","maxTokens":100000},{"label":"Request budget","maxRequests":100}]}`. They render a persistent checkpoint widget above the editor, snapshot available agents/skills/extensions/packages/tools/models/MCP configuration, and maintain source-backed evidence in the plan state; `/loop-progress clear` removes the widget. Install locally with `pi install /absolute/path/to/loop-skills`, or install the published npm package.
